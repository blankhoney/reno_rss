...
### Current Highest-Priority Task — M1.2: Reader/Ask transient-failure continuity

- **Priority:** P0 — Ask is a required core-loop action; a provider outage must not erase or distract from the current article, and its deterministic adapter must be proven before any real-provider claim.
- **Optimization purpose:** prove that a focused Reader remains useful when one Ask request fails, explains the failure truthfully, and accepts the same retry into a grounded answer with a visible citation.
- **Execution scope:** first add the smallest failing browser path for one transient Ask failure, then implement only the deterministic E2E provider contract needed to exercise 503 → retry → SSE text/citation. Preserve the article, question, current URL, existing API shape, Mock/no-spend boundary, and unexpected-console/page-error gate. Do not change model selection, real provider configuration, prompt semantics, or the annotation subsystem.
- **Acceptance standard:** the scenario must observe (1) article title/body remain visible after failure, (2) a source-specific recovery message, (3) retry of the same action, (4) a grounded answer with a usable citation, and (5) no console/page error beyond the precisely allowlisted injected 503. All-success Ask evidence alone is insufficient.
- **Verification method:** demonstrate the scenario fails before the fixture/contract exists; run it after the smallest repair, then the affected Reader Node suite, production build, and Chromium suite. Record exact SHA, fixture/state, command exits, and no-spend boundary; do not mark A-02/A-15 `PASS` until their full matrices are closed.
- **Relationship to existing goals:** this advances M1/A-02 state truth and A-15 provider safety without changing the real MiniMax integration. It preserves the reader context on which later Keep/annotation integrity work depends.
- **Reversibility:** keep Mock E2E controls and the test together as one fixture-only slice; reverting it changes no production endpoint, schema, data, dependency, or paid provider setting.
- **Current execution observation (behavior checkpoint `29799d526297114a604da2eea1402c59dfcc21b5`):** the focused M1.2 slice adds a resettable one-time Ask 503, a deterministic SSE answer/citation, and the browser scenario. It first failed because the E2E proxy had no Ask contract; final focused Chromium, 189 Reader Node tests, production build, and 47 Chromium scenarios pass. This is accepted evidence for one A-02/A-07/A-15 state slice, not completion of any row.
- **Immediate execution handoff:** select the next highest user-loss state from Keep/annotation restoration or Review/Research/Export, beginning with deterministic fixture inventory and a smallest failing scenario. Preserve both prior M1 slices and revert only the next focused candidate on failure.
...
Current checkpoint (2026-07-26): behavior revision `29799d526297114a604da2eea1402c59dfcc21b5` is based on merged main `49b2ecf2445f7995488e9a7c25346f4e4989ce8b`; M1.2 recorded a test-first failure and final passes for 189 Reader Node tests, production build, and 47 Chromium scenarios. It proves one Ask 503 is source-specific, does not remove the active article, and retries to a grounded cited answer without real-provider spend. M1.1 remains a prior green Daily slice. Both are evidence for A-02/A-07/A-15, not completion of any row. Durable records: `output/evidence/m1-daily-partial-failure-2026-07-26.json`, `output/evidence/m1-reader-ask-retry-2026-07-26.json`.
