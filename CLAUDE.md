...
- **content quality / feed demotion**: `src/lib/articles/contentQuality.ts` classifies full vs partial (RSS fragment / blocked page) and the API/worker owns fetched-content replacement. `feedHidden` / `feedQualityScore` are reserved client fields; the API does not currently emit them.
