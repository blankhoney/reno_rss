...
def load_report(path: Path) -> dict[str, Any]:
    ...
    if not isinstance(report, dict) or report.get("schemaVersion") != 1:
        raise ValueError(f"{path} is not a supported schemaVersion 1 report")
    return report


def latency_metrics(report: dict[str, Any]) -> dict[str, float]:
    ...
    for entry in entries:
        ...
        p95 = entry.get("summary", {}).get("durationMs", {}).get("p95")
...
def main() -> None:
    ...
    latency = compare(
        latency_metrics(baseline), latency_metrics(candidate), arguments.max_regression, False
    )
    throughput = compare(
