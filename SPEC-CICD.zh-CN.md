...
- **Trusted remote deploy**：`.github/scripts/remote-deploy.sh` 在锁内校验 manifest 及 bundle 内嵌的 probe/edge/rollback contract，抓取 `refs/heads/main`，要求 control-plane SHA 等于抓取的 main tip，且 operation SHA 是其 ancestor。生产事务随后先创建一份排他目录备份并验证 checksum；在此之前不会恢复 edge、登录 registry、拉取镜像、激活或迁移。之后才恢复并验证共享 edge、校验 digest-qualified image/OCI revision，并执行 activation、smoke、runtime check 与必要的 compensation。
- **Production backup gate**：锁内远程事务是每次发布生产备份的唯一创建者，并写出 mode-0600 evidence，绑定 workflow operation SHA、backup directory、checksum file 与 checksum-file digest。`infra/scripts/deploy.sh` 不再创建第二份备份，而是在任何 Compose pull/up、migration 或 edge activation 前重新验证 evidence 与全部 dump checksum。证据缺失、变化、目录碰撞或路径不安全都会停止事务，并保持当前 runtime 不变。
- **Receipt 与 probe**：成功事务记录 `pre-mutation`、`pre-activation` 与 `post-activation` 或 `post-rollback`。activation 后的 compensation 同样包含 `pre-activation`；在该 gate 之前失败时，严格只保留 `pre-mutation` 与 `post-compensation`，不伪造 `pre-activation` receipt。每份 receipt 绑定 contract version、`owner`、operation SHA、workflow run、phase、实际 runtime SHA、UTC timestamp、固定 RSS/Blog URL 结果及 Caddy/edge 状态。probe 使用有界 HTTPS GET 与固定 allowlist：RSS 必须通过 auth redirect 后最终 HTTPS 可达，Blog 必须 TLS 公开返回 `200`；Docker JSON inspect 必须证明 Caddy 同时属于 `myrss-app` 与 `brianstorm-edge`、driver/config/upstream 正常、production Blog 成员正确，且 staging web 不在 production edge。未知 phase/key、非法 URL 或任一站点/edge 失败都不得写成功 receipt，并阻止晋级。
