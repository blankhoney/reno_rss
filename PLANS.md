...
| M1.3b internal-only metrics boundary | A-05, supports A-12/A-15 | Both public AI Reader hosts deny exact `/api/metrics` with 404; the worker smoke probe uses the environment API alias on the shared app network | Static route-order contract, shell syntax, API metrics 3/3, all Compose renders, and staging deployment proof pass: internal scrape OK/public 404 |
...
| Status | IN_PROGRESS — PR candidate CI/staging and D-04 live proof pass at `73792c63`; evidence checkpoint, merge, and post-merge main proof remain |
...
| A-02 | IN_PROGRESS | Candidate local and CI gates are green; live PostgreSQL conditional tests and cross-browser/project performance gates remain |
| A-03 | PASS | Production audit 0, full audit 1 development low, regression gates green, and pinned Trivy vulnerability/secret scan passes in run `30172544347` |
...
| A-05 | IN_PROGRESS | Staging live proof passes: internal worker-to-alias scrape OK, public metrics 404, Admin 403, local secret mode 0600, Trivy green; production remains fail-closed/unavailable and VPS secret modes are unverified |
| A-06 | IN_PROGRESS | Versioned anchor capture/API/editor-focus paths pass; repeated-quote, markup, refresh, remaining input/retry, and session-isolation matrix remains |
...
| A-12 | IN_PROGRESS | PR run `30172544347` is green; exact SHA images and staging deploy/smoke are proven. Post-merge main, rollback, and GHCR cleanup remain |
...
| D-02 dependency remediation | Implemented, pushed, CI-proven (`c61cd6a6`) | Retain overrides until Next's declared ranges catch up |
| D-03 production intent | Pending maintainer | Staging/local work and read-only public probes only |
| D-04 metrics trust boundary | Implemented, pushed, staging-proven (`cafc4cc1`) | Exact public 404 and internal environment-alias scrape both pass in run `30172544347`; production remains outside scope |
...
| 2026-07-26 | PR #15 exact-candidate CI and staging proof | Run `30172544347` passes test/build/Compose/Trivy, three-image publication, deployment, and smoke at `73792c63`; internal metrics scrape passes and public path is 404 | No new architecture change; the existing immutable-image and staging workflow supplied live proof for D-02/D-04/anchor integration | Real MiniMax inference skipped to avoid credit spend; Node action deprecation warning, merge/main proof, rollback, and GHCR cleanup remain | Commit and push the evidence checkpoint; require its new run before merge |
