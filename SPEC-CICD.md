...
- **Trusted remote deploy**: `.github/scripts/remote-deploy.sh` validates the locked manifest and bundled probe/edge/rollback contracts, fetches `refs/heads/main`, requires the control-plane SHA to be that fetched main tip, and requires the operation SHA to be its ancestor. For production it then creates and checksum-verifies one exclusive backup before edge recovery, registry login, image pull, activation, or migration. It next restores/verifies the shared edge, verifies digest-qualified images and OCI revision, and runs activation, smoke, runtime checks, and compensation when needed.
- **Production backup gate**: the locked remote transaction is the only creator of the per-release production backup. It writes mode-0600 evidence bound to the workflow operation SHA, backup directory, checksum file, and checksum-file digest. `infra/scripts/deploy.sh` never creates a second backup; before any Compose pull/up, migration, or edge activation it revalidates that evidence and all dump checksums. Missing, changed, colliding, or unsafe evidence stops the transaction with the current runtime intact.
- **Receipts and probes**: successful transactions record `pre-mutation`, `pre-activation`, and `post-activation` or `post-rollback`. Compensation after activation also records `pre-activation`; a failure before that gate records the strict early set `pre-mutation` plus `post-compensation` without fabricating a `pre-activation` receipt. Each receipt binds contract version, `owner`, operation SHA, workflow run, phase, actual runtime SHA, UTC timestamp, fixed RSS/Blog URL results, and Caddy/edge state. The probe uses bounded HTTPS GETs with an allowlist: RSS must satisfy auth redirect then final HTTPS reachability, Blog must return public `200` over TLS, and Docker JSON inspection must prove Caddy's `myrss-app` and `brianstorm-edge` memberships, bridge drivers, loaded config, reachable RSS/Blog upstreams, production Blog membership, and no staging web on production edge. Unknown phases, unknown keys, invalid URLs, or any failed site/edge check produce no success receipt and block promotion.
