...
- **Rollback provenance blocker**: the current rollback artifact carries `deploy_sha` but no trusted `DEPLOY_REF` or workflow-run/ref provenance claim. After `main` advances, an old SHA cannot satisfy the remote contract requiring a trusted `DEPLOY_REF` plus `FETCH_HEAD^{commit}` to equal `DEPLOY_SHA`. Rollback is not enabled or usable until a trusted orchestrator defines trusted ref/provenance resolution or a safe SHA-based fetch contract; do not restore arbitrary `git_ref` input.
