...
`infra/caddy/Caddyfile` 是 bind mount 到 Caddy 容器里的。宿主机文件更新后，运行中的 Caddy 不会自动重新加载；`docker compose up -d` 也可能因为服务定义未变化而不重建容器。部署脚本需要在 edge 更新后执行 `caddy validate` 和 `caddy reload`，否则新域名会继续 TLS handshake failure。
