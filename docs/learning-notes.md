...
## AI Reader Web Task 4：reader-web 壳与 Docker 镜像

> 与 `docs/superpowers/plans/2026-05-13-ai-reader-web-implementation-plan.md` **Task 4** 对齐。

### 做了什么

- 在 `apps/reader-web` 落地 Next.js App Router 根布局、首页三栏壳、全局样式，以及 `tsconfig.json`、`next.config.ts`（`output: "standalone"`）。
- 添加多阶段 `Dockerfile`（Node 22 Alpine：`deps` → `builder` → `runner`），运行时只带上 standalone 产物、`.next/static` 和 `public`。

### 关键概念

**`output: "standalone"`**
Next 会把运行所需的最小 `node_modules` 子集打进 `.next/standalone`，容器里不需要整份依赖树，镜像更小、启动更贴近生产。

**Docker 为什么要分三段**
`deps` 只装依赖且可利用缓存；`builder` 在干净环境里 `npm run build`；`runner` 不包含源码和 devDependencies，减少攻击面和体积。

**三栏桌面壳为什么在 CSS 里用 grid**
侧栏 / 列表 / 阅读器是工作台的主结构，大屏用固定+弹性列组合；窄屏再用媒体查询收成单列堆叠，先保证桌面效率再兼顾手机。

### 可以在学习 session 里追问的问题

- standalone 模式和 `next start` 直接跑有什么区别？各适合什么部署？
- Alpine 镜像里还有哪些常见坑（例如原生模块、`libc` 差异）？
- 为什么 Dockerfile 里用 `npm ci` 而不是 `npm install`？

---

## AI Reader Web Review：Next 依赖安全升级

### 做了什么

- `apps/reader-web` 从 `next@15.1.7` 升级到 `next@15.5.18`，同步更新 `package-lock.json`。
- 保持在 Next 15 主线内修复 review blocker，避免把本轮变成 Next 16 大版本迁移。
- npm registry 中 `next@15.5.18` 无 deprecated 标记，原 lockfile 中的 CVE-2025-66478 deprecation 不再出现。

### 关键概念

**安全修复优先选择最小兼容升级**
当漏洞修复可以在同一主线内完成时，先选择同主线最新补丁版本，降低框架行为变化对部署和页面构建的影响。

**audit 自动修复不一定可信**
本轮 `npm audit --audit-level=high --omit=dev` 通过；剩余 PostCSS 报告为 moderate，`npm audit fix --force` 会建议降到旧 Next 9，属于不适合当前项目的破坏性路径。
