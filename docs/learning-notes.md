...
## 通用 Agent 规则入口与 VPS 诊断 Prompt

### 做了什么

明确了 agent 规则入口，并补齐通用规则文件：
- `AGENTS.md`：作为 Codex 和通用 coding agent 的仓库入口，记录四条开发规则、学习笔记维护要求和 VPS secret 安全要求。
- `.cursor/rules/vps-agent-diagnostics.mdc`：保留为 Cursor 专用辅助规则，不再假设 Codex 会自动读取。
...
**AGENTS.md 是通用入口，.cursor/rules 是 Cursor 入口**
Codex 不应依赖 `.cursor/rules` 自动加载。把通用规则放在仓库根目录 `AGENTS.md`，能让 Codex、其他 coding agent 和人工维护者更容易找到同一份约束。
...
- Codex、Cursor、Claude 对 `AGENTS.md` / `.cursor/rules` / `CLAUDE.md` 的读取方式有什么区别？
