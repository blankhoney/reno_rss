...
> 用法：每完成一个 Task，先回答“自测问题”，再进入下一个 Task。<br>
...
Run: `rg "services:|networks:|edge|app|data|postgres_data|authelia_data" infra/compose/docker-compose.base.yml infra/compose/docker-compose.edge.yml .env.example`<br>
...
Run: `rg "default_policy: deny|rules:|AUTHELIA_SESSION_SECRET_FILE|AUTHELIA_STORAGE_ENCRYPTION_KEY_FILE" infra/authelia/configuration.yml infra/compose/docker-compose.base.yml`<br>
...
Run: `rg "forward_auth|authelia-prod|authelia-staging|miniflux-prod|miniflux-staging" infra/caddy/Caddyfile -n`<br>
...
Run: `rg "postgres:|ports:" infra/compose/docker-compose.base.yml -n`<br>
Expected: 只有 edge Caddy 有 80/443 映射，`postgres` 无端口映射

- [ ] **Step 5: 本地 compose 配置检查**

Run: `docker compose -f infra/compose/docker-compose.edge.yml -f infra/compose/docker-compose.base.yml config`<br>
...
Run: `pytest apps/scorer-worker/tests/test_scoring.py::test_score_payload_shape -v`<br>
...
Run: `pytest apps/scorer-worker/tests/test_scoring.py -v`<br>
Expected: PASS
...
Run: `pytest apps/scorer-worker/tests/test_repository.py -v`<br>
...
Run: `chmod +x infra/scripts/*.sh && shellcheck infra/scripts/*.sh`<br>
...
Run: `rg "runbooks|audit-checklist|readyz" docs -n`<br>
...
Run: `rg "PASS|FAIL|N/A|Go / No-Go" docs/superpowers/specs/2026-05-11-rss-audit-checklist.md -n`<br>
...
1. **Spec coverage:** 已覆盖认证、安全、数据模型、备份恢复、CI/CD、staging/prod 隔离与审计门槛。<br>
2. **Placeholder scan:** 仅保留必要骨架示例，无 `TODO/TBD`。<br>
3. **Type consistency:** `tenant_id/miniflux_entry_id/content_hash/model_version` 在任务中统一。<br>
