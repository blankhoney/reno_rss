...
async def test_saved_searches_reject_sort_the_article_list_cannot_execute(client):
    await client.post("/api/auth/login", json={"display_name": "FilterUser"})
    response = await client.put(
        "/api/saved-searches",
        json={"items": [{"name": "bad", "sort": "oldest_magic"}]},
    )
    assert response.status_code == 422
