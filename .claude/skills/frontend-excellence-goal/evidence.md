...
| FEX-06 | In progress | `globals.css` defines documented semantic tokens for sticky UI, bottom navigation, popovers, Agent drawer, modal overlay, command palette, and toast. Opening Agent suppresses the selection toolbar while retaining its selection chip, avoiding competing action layers; mobile Toast reserves space above selection or an expanded Agent. | Chromium at 375×812 proves selection toolbar is above nav, Toast is above selection, then Agent hides selection while Toast moves above Agent. At 390×844 and 768×1024, Toast is above Agent and Agent is above nav. Full simultaneous-layer matrix remains pending. |
| FEX-07 | In progress | Shared `--mobile-bottom-nav-offset` governs body, Focus reader, Agent and Toast; selection toolbar now uses the same offset at ≤900px; module drawer inerts app/background and locks scroll. | Chromium at 375×812 proves drawer is above bottom nav and background cannot be interacted with; real mouse selection proves toolbar bounds stay within viewport and above nav, Toast avoids it, then Agent retains the selection chip. At 390×844 and 768×1024, Agent and Toast remain separate from bottom navigation with no horizontal overflow. Per-control/per-viewport matrix remains pending. |
