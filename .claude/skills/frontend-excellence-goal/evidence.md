...
| FEX-17 | Pending — Daily Intelligence slice passed; other primary views remain | Daily Intelligence now distinguishes per-entry/radar/source loading, empty and error states, retains successful sections under partial failure, and has a reload action. | Chromium uses the local missing-endpoint fixture to prove failed entries render `加载失败` rather than false `暂无条目`; 40-test suite passed. Remaining: Review/workbench/reader and smaller product panels. |
