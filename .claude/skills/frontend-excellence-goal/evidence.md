...
| FEX-06 | In progress | `globals.css` defines documented semantic tokens for sticky UI, bottom navigation, popovers, Agent drawer, modal overlay, command palette, and toast. | Chromium at 375×812 confirms module drawer layers above bottom navigation. Agent/selection/Toast simultaneous-state proof remains pending. |
| FEX-07 | In progress | Shared `--mobile-bottom-nav-offset` governs body, Focus reader, Agent and Toast; module drawer now inerts app/background and locks scroll. | Chromium at 375×812 proves drawer is above bottom nav and background cannot be interacted with. Agent, selection, Toast and 390/768 proof remain pending. |
| FEX-08 | In progress | Focus-specific desktop grid rule is constrained to `min-width: 901px`; dual-pane CSS now explicitly stacks its secondary content at ≤900px. | Chromium at 899×900 confirms nonzero workbench column and no horizontal overflow when dual-pane is enabled. Required 901px continuity remains pending. |
| FEX-09 | In progress | `focusReaderLayout` explicitly contains primary article content and a notes/comparison secondary pane; desktop dual-pane uses readable primary/secondary columns while narrow layouts preserve the secondary pane after content. | Chromium at 1440×1000 confirms grid primary wider than secondary and no overlap; at 899×900 confirms block stacking after primary and no horizontal overflow. Phone/tablet Scan/Focus/Keep matrix remains pending. |
